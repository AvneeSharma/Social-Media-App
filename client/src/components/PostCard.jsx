    ...
    return (
        <div className='w-full py-3'>
            <div className='flex gap-3 items-center mb-1'>
                <Link to={"/profile/" + reply?._userId?._id}>
                    <img src={reply?.userId?.profileUrl ?? NoProfile} alt={reply?.userId?.firstName} className='w-10 h-10 rounded-full object-cover'></img>
                </Link>
...
const CommentForm= ({ user, id, replyAt, getComments }) => {
    ...
    return <form onSubmit={handleSubmit(onSubmit)} className='w-full border-b border[#66666645]'>
        ...
        <img src={user?.profileUrl ?? NoProfile} alt='User Image' className='w-10 h-10 object-cover rounded-full'></img>
        ...
        <div className='flex items-end justify-end pb-2'>
              {
                  loading ? <Loading /> : <CustomButton type="submit" containerStyles="bg-[#0444a4] text-white py-1 px-3 rounded-full font-semibold text-sm" title="Submit" />
              }
        </div>
  ...
  return (
    <div className='mb-2 bg-primary p-4 rounded-xl'>
      <div className='flex gap-3 items-center mb-2'>
        ...
        <img src={post?.userId?.profileUrl ?? NoProfile} alt={post?.userId?.firstName} className='w-12 h-12 md:w-14 h-14 object-cover rounded-full' />
        </Link>
      ...
      <div>
        ...
        {
            post?.image && (
                <img src={post?.image} alt='post image' className='w-full mt-2 rounded-lg'></img>
            )
        }
      </div>
      ...
      <div className='mt-4 flex justify-between items-center px-3 py-2 text-ascent-2 text-base border-t border-[#66666645]'>
        ...
        {
            user?._id === post?.userId?._id && <div className='flex gap-1 items-center text-base text-ascen-1 cursor-pointer' onClick={() => deletePost(post?._id)}>
                ...
                loading ? (
                    ...
                    comments?.map((comment) => (
                        <div className='w-full py-2' key={comment?._id}>
                            <div className='flex gap-3 items-center mb-1'>
                                <Link to={"/profile/" + comment?._userId?._id}>
                                <img src={comment?.userId?.profileUrl ?? NoProfile} alt={comment?.userId?.firstName} className='w-10 h-10 rounded-full object-cover'></img>
                                </Link>
                                <div>
                                    <Link to={"/profile/" + comment?._userId?._id}>
                                        <p className='font-medium text-base text-ascent-1'>
                            ...
                            <div className='py-2 px-8 mt-6'>
...

                                {
                                    showReply === comment?.replies?._id && comment?.replies?.map((reply) => (
                                        <ReplyCard reply={reply} user={user} key={reply?._id} handleLike={() => handleLike(
