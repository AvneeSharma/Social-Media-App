...
import { deletePost, fetchPosts, getUserInfo, handleFileUpload, likePost, sendFriendRequest } from '../utils';
    ...
    <div className='home w-full px-0 lg:px-10 pb-20 2xl:px-40 bg-bgColor lg:rounded-lg h-screen overflow-hidden'>
      ...
      <div className='w-full flex gap-2 lg:gap-4 pt-5 pb-10 h-full'>
        ...
        <div className='flex-1 h-full px-4 flex flex-col gap-6 overflow-y-auto rounded-lg'>
          <form className='sbg-primary px-4 rounded-lg' onSubmit={handleSubmit(handlePostSubmit)}>
            <div className='w-full flex items-center gap-2 py-4 border-b border=[#66666645]'>
              <img src={user?.profileUrl ?? NoProfile} alt={'User Image'} className='w-14 h-14 object-cover rounded-full' />
              <TextInput styles='w-full rounded-full py-5' placeholder="What's on your mind..." name="description" register= {register("description", {
            ...
            <div className='flex items-center justify-between py-4'>
              ...
              <label htmlFor='vgifUpload' className='flex items-center gap-1 text-base text-ascent-2 hover:text-ascent-1 cursor-pointer'>
                <input type='file' onChange={(e) => setFile(e.target.files[0])} className='hidden' id='vgifUpload' data-max-size='5120' accept='.gif' />
                <BsFiletypeGif />
                <span>Gif</span>
              </label>
              ...
              <div>
              {
                  posting ? <Loading /> : <CustomButton type="submit" containerStyles="bg-[#0444a4] text-white py-1 px-6 rounded-full font-semibold text-sm" title="Post" />
              }
              </div>
            </div>
                ...
                friendRequest?.map(({_id, requestFrom: from }) => (
                  <div key={_id} className='flex items-center justify-between'>
                    ...
                    <div className='flex gap-1'>
                      <CustomButton title='Accept' containerStyles='bg-[#0444a4] text-xs text-white px-1.5 py-1 rounded-full' onClick={() => acceptFriendRequest(_id, "Accepted")}/>
                      <CustomButton title='Deny' containerStyles='border border-[#666] text-xs text-ascent-1 px-1.5 py-1 rounded-full' onClick={() => acceptFriendRequest(_id, "Denied")}/>
                    </div>
                  </div>
                ))
              }
            ...
            <div className='w-full flex flex-col gap-4 pt-4'>
              {
                suggestedFriends?.map((friend) => (
                  <div className='flex items-center jistify-between' key={friend._id}>
